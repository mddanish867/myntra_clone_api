...
namespace myntra_clone_api.Controllers
{
    ...
    public class CategoriesController : ControllerBase
    {
        ...
        public IActionResult GetCategoryAsync([FromQuery] string maincategory, [FromQuery] string category, [FromQuery] string subcategory)
        {
            try
            {
                var categoriesDto = GetCategoryInternal(maincategory, category, subcategory);
                return Ok(categoriesDto);
            }
            catch (Exception ex)
            {
        ...
        private IEnumerable<CategoriesDTO> GetCategoryInternal(string maincategory, string category, string subcategory)
        {
